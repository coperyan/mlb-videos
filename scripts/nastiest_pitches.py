...
bad_pitch_ids = ["718721|53|3", "718430|66|1", "", ""]

# Remove any bad pitch IDs
pitches.df = pitches.df[~(pitches.df["pitch_id"].isin(bad_pitch_ids))]

# Remove calls with speed <= 65 mph
pitches.df = pitches.df[(pitches.df["release_speed"] >= 60)]

# Filter on only called strikes and swinging strikes?
pitches.df = pitches.df[
    (pitches.df["description"].isin(["called_strike", "swinging_strike"]))
]
pitches.refresh_index()


# Now rank by total break?
pitches.rank_pitches(
    order_by="horizontal_break", ascending=False, name="horizontal_break_rank"
)
pitches.rank_pitches(
    order_by="vertical_break", ascending=False, name="vertical_break_rank"
)
...
# Limit to top 30..
pitches.df = pitches.df[
    (pitches.df["horizontal_break_rank"] <= 25)
    | (pitches.df["vertical_break_rank"] <= 25)
]
pitches.refresh_index()
...
pitches.save_df(name="nasty_pitches")
