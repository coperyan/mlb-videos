...
class Video:
    ...
    def __init__(
        self, p=None, dl: bool = True, path: str = None, compress: bool = False
    ):
        """ """
        self.pitch = p
    ...
    def search(self):
        """ """
        self.play_id = Search(pitch=self.pitch).get_play_id()

    def download(self):
        """ """
        if self.play_id:
            self.file_path = Clip(
                play_id=self.play_id, download=True, path=self.path
            ).get_file_path()
    ...
    def render_compressed(self):
        ...
        n_save_path = os.path.join(
            self.path,
            os.path.basename(self.file_path).replace(".mp4", "_compressed.mp4"),
        )
        comp_clip = VideoFileClip(self.file_path, fps_source="fps")
        comp_clip.write_videofile(n_save_path, fps=FPS_DEFAULT)
...
class VideoCompilation:
    ...
    def create_compilation(self):
        ...
        for clip in self.clips:
            self.compclips.append(VideoFileClip(clip, fps_source="fps"))
        self.comp = concatenate_videoclips(self.compclips, method="compose")
        self.comp.write_videofile(self.file_path, fps=FPS_DEFAULT)
        return self.file_path
