...
class Pitches:
    ...
    def calculate_movement(self):
        ...
        self.df[PM_COLS] = self.df.swifter.apply(
            lambda x: calculate_movement(x), axis=1, result_type="expand"
        )
    ...
    def add_player_info(
        self, batters: bool = True, pitchers: bool = True, socials: bool = False
    ):
        """ """
        logging.info(
            "Adding player info.."
        ...
        for player_type in [x[0] for x in player_iterations if x[1]]:
            ...
            player_df.rename(
                columns={c: f"{player_type}_{c}" for c in player_df.columns.values},
                inplace=True,
            )
    ...
    def add_team_info(self):
        ...
        for team_type in team_iterations:
            ...
            team_df.rename(
                columns={c: f"{team_type}_{c}" for c in team_df.columns.values},
                inplace=True,
            )
            self.df = self.df.merge(
    ...
    def rank_pitches(
        ...
        if name is None:
            ...
            wrk[name] = wrk[order_by].rank(method="first", ascending=ascending)
    ...
    def save_df(self, name: str = "pitches"):
        """ """
        self.df.to_csv(
            os.path.join(self.path, DF_SUBFOLDER, f"{name}.csv"), index=False
        )
