...
def calculate_ball(d):
    """ """
    if (d.X1 < d.X and d.X < d.X2) and (d.Y1 < d.Y and d.Y < d.Y2):
...
def calculate_miss(p):
    ...
    """
    if (
        any(v == 0 for v in (p.plate_x, p.plate_z, p.sz_bot, p.sz_top))
        or p.description not in CALLS
    ):
        return default_vals()
    else:
        d = {x: getattr(p, x) for x in SRC_COLS}
