...
class Media:
    ...
    def _iteration_upload(self):
        ...
        while bytes_sent < self.file_bytes:
            ...
            if req.status_code < 200 or req.status_code > 299:
                logging.critical(
                    f"Media Upload Status Code: {req.status_code} - Message: {req.text}"
                )
            ...
            logging.info(
                f"Segment: {segment_id} - {bytes_sent} of {self.file_bytes} uploaded.."
            )
    ...
    def _finalize_upload(self):
        ...
        """
        r_data = {"command": "FINALIZE", "media_id": self.media_id}
        req = requests.post(MEDIA_URL, data=r_data, auth=self.oauth)
    ...
    def _check_status(self):
        ...
        r_params = {"command": "STATUS", "media_id": self.media_id}
